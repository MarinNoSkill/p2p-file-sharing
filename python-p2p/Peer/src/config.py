...
class ConfigManager:
    ...
    def _load_config(self) -> Dict[str, Any]:
        """Carga la configuración desde archivo JSON/YAML o variables de entorno"""
        ...
        if os.path.exists(self.config_path):
            ...
            # Si no existe archivo, crear configuración vacía
            config = {}
    ...
    def _override_with_env_vars(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config['peer']['peer_id'] = os.getenv('PEER_ID', config['peer'].get('peer_id', 'peer1'))
        config['peer']['username'] = os.getenv('PEER_USERNAME', config['peer'].get('username', 'usuario1'))
        config['peer']['password'] = os.getenv('PEER_PASSWORD', config['peer'].get('password', 'password1'))
        
        # Configuración de red
        if not config.get('network'):
            config['network'] = {}
        
        config['network']['host'] = os.getenv('PEER_HOST', config['network'].get('host', 'localhost'))
        config['network']['rest_port'] = int(os.getenv('PEER_REST_PORT', config['network'].get('rest_port', 8001)))
        config['network']['server_url'] = os.getenv('SERVER_URL', config['network'].get('server_url', 'localhost:50051'))
        
        # Configuración de archivos
        if not config.get('files'):
            config['files'] = {}
        
        config['files']['shared_directory'] = os.getenv('SHARED_DIRECTORY', config['files'].get('shared_directory', './shared_files'))
        config['files']['allowed_extensions'] = config['files'].get('allowed_extensions', ['.txt', '.json', '.py', '.md', '.log'])
        config['files']['max_file_size_mb'] = config['files'].get('max_file_size_mb', 100)
        ...
        config['logging']['format'] = config['logging'].get('format', '%(asctime)s - %(name)s - %(levelname)s - %(message)s')
        config['logging']['file'] = config['logging'].get('file', 'peer.log')
